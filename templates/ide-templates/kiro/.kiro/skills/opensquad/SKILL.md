...
5. Use web fetch on their URL + web search with their company name to research:
...
When the user types `/opensquad` or asks for the menu, present the following options as a numbered list and ask the user to reply with the option number:
...
If the user selects "More options", present a second numbered list:
...
2. Present the skills submenu as a numbered list and ask the user to reply with the option number:
   1. **View installed skills** — See what's installed and their status
   2. **Install a skill** — Browse the catalog and install
   3. **Create a custom skill** — Create a new skill (uses opensquad-skill-creator)
   4. **Remove a skill** — Uninstall a skill
...
- **All menus and checkpoint questions use numbered lists.** When presenting a dynamic list (squads, skills, agents, etc.) and only 1 item exists, still present it as a numbered list and add a fallback option like "Cancel" or "Back to menu". If 0 items exist, skip the list entirely and inform the user directly.
